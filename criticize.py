...
def interpret_labels(regular_labels: list) -> list:
    judgy_labels = [
        ...
        "wasting time",
        "wasting money",
        "shenanigans and gallivanting",
        "not calling enough",
        "spending too much time with friends",
        "don't have enough friends",
        "not studying",
        "spending too much time on ur phone",
        "too skinny",
        "overdue for the dentist",
        "still not a homeowner",
        # "not skinny enough",
        "still not a doctor",
        "is that ur boyfriend/girlfriend?",
        "out too late like a criminal",
        "not making enough money",
        "not eating healthy enough",
    ]
    relevant_judgy_label = ["still not married yet"]
    for judgylabel in judgy_labels:
        ...
        for regular_label in regular_labels:
            ...
            rlast_hidden_states = routputs.hidden_states[
                -1
            ]  # Get the last hidden states
            # Get the mean of the last hidden states for the regular label
            regular_label_embedding = (
                rlast_hidden_states.mean(dim=1).squeeze().detach().numpy()
            )

            # Calculate cosine similarity
            cosine_similarity = np.dot(label_embedding, regular_label_embedding) / (
                np.linalg.norm(label_embedding)
                * np.linalg.norm(regular_label_embedding)
            )
